-- Align backend perfect-run time limit with frontend timer (3 minutes).
-- Without this, clean runs between 101s and 180s are wrongly marked non-perfect,
-- which removes PERFECT badges and can downgrade level progression.
...
DECLARE
    ...
    v_level_time_limit INT := 180; -- 3 minutes
    v_new_level_cap INT;
    v_on_time BOOLEAN;
    v_no_errors BOOLEAN;
    v_status TEXT;
